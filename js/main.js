...
renderer.setClearColor( 0Xb7c3f3 );
...
class Doll{
  ...
  async start(){
    this.lookBackward()
    await delay((Math.random() * 1267) + 1267);
    this.lookForward()
    await delay((Math.random() * 756) + 756);
    this.start()
  }
}
...
function startGame() {
  gameStat = "started"
  let progressBar = createCube({w: 5.1, h: .1, d: 1}, 0);
  progressBar.position.y = 3.35;
  gsap.to(progressBar.scale, {x: 0, duration: time_limit, ease: "none"});
  doll.start();
  setTimeout(() => {
    if (gameStat != "Over!"){
      text.innerText = "You are out of time!"
      gameStat = "Over!"
    }
  }, time_limit * 1000);
}
